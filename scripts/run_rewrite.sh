...
if [ -z "${SHARE_TEXT_RAW}" ]; then
  echo "Usage: ${0} 'share text with douyin url'"
  exit 1
fi
...
echo "[3/4] create rewrite job"
COPY_REQ="$(node -e 'console.log(JSON.stringify({
  sourceText: process.argv[1],
  mode: "rewrite",
  variantCount: 3,
  strictness: "strict"
}))' "${TRANSCRIPT_TEXT}")"

COPY_RESP="$(curl -sS -X POST "${BASE_URL}/api/copy/variants" \
  -H "Content-Type: application/json" \
  -d "${COPY_REQ}")"
JOB_ID="$(json_get "${COPY_RESP}" "jobId")"
echo "jobId=${JOB_ID}"

echo "[4/4] polling rewrite job"
FINAL=""
for i in $(seq 1 60); do
  RESP="$(curl -sS "${BASE_URL}/api/jobs/${JOB_ID}")"
  STATUS="$(json_get "${RESP}" "status" || true)"
  echo "  rewrite[${i}] status=${STATUS}"
